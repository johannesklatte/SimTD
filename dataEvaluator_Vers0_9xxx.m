        ...
        for iFile = 1 : 1 : dirLengthUntar
            if(isequal(MyDirInfoUntared(iFile).isdir, 0))
                ...
                while ~feof(fileID)
                    ...
                    %chunkResultArray = cellfun(@(x) textscan(x, '%s', chunkSize, 'CommentStyle', '##', 'delimiter', ','), dataChunk);
                    ...
                    for i = 1 : 1 : chunkSize
                        try
                            if(strcmp(dataChunk{i,1}(1,32:42), 'm_IVS_AU_VA'))
                                ...
                                %tempEntry = transpose(tempEntry{1,1});
                                %tempEntry = vertcat(tempEntry{1,1}, dummy);
                                %tempEntry = tempEntry(1:22, 1);
                                %tempEntry = transpose(tempEntry);
                                [length, j2] = size(tempEntry{1,1});
                                for j = 1 : 1 : length
                                    chunkResultTable{entryCounter,j} = tempEntry{1,1}{j,1};
                                end                                
%                                 %01
%                                 if(strcmp(tempEntry{1,1}{3,1},'m_IVS_AU_VAPIClient_EngineSpeed'))
%                                     entryCounter = entryCounter + 1;
%                                     chunkResultArray{entryCounter, 1} = tempEntry{1,1}{2,1};
%                                     chunkResultArray{entryCounter, 2} = tempEntry{1,1}{5,1};
%                                 %02    
%                                 elseif(strcmp(tempEntry{1,1}{3,1},'m_IVS_AU_VAPIClient_VehicleSpeed'))
%                                     entryCounter = entryCounter + 1;
%                                     chunkResultArray{entryCounter, 1} = tempEntry{1,1}{2,1};
%                                     chunkResultArray{entryCounter, 3} = tempEntry{1,1}{5,1};
%                                 %03        
%                                 elseif(strcmp(dataChunk{i,1}{3,1}, 'm_IVS_AU_VAPIClient_SimTD_FilteredPosition'))
%                                     entryCounter = entryCounter + 1;
%                                     chunkResultArray{entryCounter, 1} = tempEntry{1,1}{2,1};
%                                     chunkResultArray{entryCounter, 4} = tempEntry{1,1}{5,1};
%                                     chunkResultArray{entryCounter, 5} = tempEntry{1,1}{5,1};
%                                     chunkResultArray{entryCounter, 6} = tempEntry{1,1}{5,1};
%                                     chunkResultArray{entryCounter, 7} = tempEntry{1,1}{5,1};
%                                     chunkResultArray{entryCounter, 8} = tempEntry{1,1}{5,1};
% 
%                                 %04
